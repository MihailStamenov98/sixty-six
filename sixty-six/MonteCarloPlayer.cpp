...
double MonteCarloPlayer::treeExploaringFirstMove(Node* root, vector<Card> myCards, vector<Card> remainingCards)
{
	if (root->n == 0)
	{
		root->children = createChildren(myCards);
		int randIndex = getRandomIndex(root->children.size());
		Card firstCard = myCards[randIndex];
		myCards.erase(myCards.begin() + randIndex);
		root->children[0]->t = otherPlayerSimulationResposnse(firstCard, myCards, remainingCards);
	}
}
